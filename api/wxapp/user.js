...
router.post('/login', async ctx => {
  let user = await UserModel.findOne({ openid: ctx.userOpenid }).select(
    'name avatar openid begin_date'
  )
  if (user) {
    ctx.body = {
      code: 1,
...
router.get('/getloverinfo', async ctx => {
  let loverUser = await UserModel.findOne({ openid: ctx.userOpenid }).select(
    'lover_openid'
  )
...
router.post('/addlover', async ctx => {
  ...
  await UserModel.findOneAndUpdate(
    { openid: lover_openid },
    { $set: { lover_openid: openid, openid: lover_openid } },
    { upsert: true }
  )
  const result = await UserModel.find(
    { openid: { $in: [openid, lover_openid] } },
    { begin_date: 1 }
  ).lean()
  if (result[0].begin_date && result[1].begin_date) {
    if (result[0].begin_date > result[1].begin_date) {
      await UserModel.updateMany(
        { openid: { $in: [openid, lover_openid] } },
        { $set: { begin_date: result[1].begin_date } }
      )
    } else {
      await UserModel.updateMany(
        { openid: { $in: [openid, lover_openid] } },
        { $set: { begin_date: result[0].begin_date } }
      )
    }
